    ...
    function($, Vue, atosBaseController, moment, momentConfig, atosUtil, axios, tinyImgUpload) {
        ...
        var controller = atosBaseController(momentConfig, {
            ...
            initVue: function() {
                var self = this;
                this.vue = new Vue({
                    el: '#alertBlackoutAPIUpdateRequestApp',
                    data: {
                        dataCentreId: '',
                        description: '',
                        blackoutWindowBegin: '',
                        ...
                        apiName:'alertBlackoutAPIUpdateRequest'
                    },
                    methods: {
                        doSubmit: function() {
                            var self = this;
                            if (!$.formValidator('#alertBlackoutAPIUpdateRequestForm')) { // 驗證
                                return false;
                            }

                            var requestDTO = JSON.stringify($('#alertBlackoutAPIUpdateRequestForm').serializeObject());
            ...
            initEvent: function() {
                ...
                $('#update-blackoutWindowBegin').datetimepicker({
                    //language:  'cn',
                    ...
                    var starttime = $("#update-blackoutWindowBegin").val();
                    $("#update-blackoutWindowEnd").datetimepicker('setStartDate', starttime);
                    $("#update-blackoutWindowBegin").datetimepicker('hide');
                });

                $('#update-blackoutWindowEnd').datetimepicker({
                    //language:  'cn',
                    //minView: "month",//设置只显示到月份
                    startDate: sysDate,
                    ...
                    var endtime = $("#update-blackoutWindowEnd").val();
                    $("#update-blackoutWindowBegin").datetimepicker('setEndDate', endtime);
                    $("#update-blackoutWindowEnd").datetimepicker('hide');
                });
            ...
            vaildate: function() {
                var that = this;
                $("#alertBlackoutAPIUpdateRequestForm").bootstrapValidator({
                    fields: {
                        dataCentreId: {
                            trigger: 'change',
                            validators: {
