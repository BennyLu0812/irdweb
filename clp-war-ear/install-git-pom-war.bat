...
%M2_HOME%\bin\mvn -f pom-ear.xml clean install -Dmaven.test.skip=true -D%Package_type% -DfinalName=%JOB_NAME%-%dlsVersion%.stable-%buildTime% -DbuildTime=%buildTime% -DdlsVersion=%dlsVersion% -Drun.env=%package_env% -DCOOKIE-NAME=%COOKIE-NAME% -l log-ear.log 